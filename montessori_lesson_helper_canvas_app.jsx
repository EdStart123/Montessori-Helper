...
/**
 * Montessori Lesson Helper — all‑in‑one Canvas app
 *
 * ✅ Upload from files  ✅ Use camera  ✅ Drag‑and‑drop
 * ✅ Offline guidance for core Montessori materials
 * ✅ YouTube search shortcuts for lesson videos
 * ✅ Montessori coach chat (offline fallback; optional OpenAI key)
 * ✅ Built‑in Test Panel to sanity‑check core logic
 */
...
const Card = ({ className = "", children }) => (
  <div className={`bg-white border border-zinc-200 rounded-2xl shadow-sm ${className}`}>{children}</div>
);
const SectionTitle = ({ children, right }) => (
  <div className="flex items-center justify-between mb-3">
    <h3 className="text-lg font-semibold tracking-tight text-zinc-900">{children}</h3>
    {right}
  </div>
);
const Button = ({ children, onClick, type = "button", disabled, className = "" }) => (
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`px-4 h-11 rounded-xl border border-zinc-300 bg-zinc-50 hover:bg-zinc-100 disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
  >
    {children}
  </button>
);
const PrimaryButton = ({ children, onClick, type = "button", disabled, className = "" }) => (
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`px-4 h-11 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
  >
    {children}
  </button>
);
const Input = (props) => (
  <input
    {...props}
    className={`w-full h-11 rounded-xl border border-zinc-300 px-3 bg-white focus:outline-none focus:ring-4 focus:ring-indigo-100 ${props.className || ""}`}
  />
);
const Badge = ({ children }) => (
  <span className="inline-block text-xs bg-zinc-100 text-zinc-700 px-2.5 py-1 rounded-full border border-zinc-200">{children}</span>
);

function Placeholder({ children }) {
  return (
    <div className="rounded-xl border border-dashed border-zinc-300 p-6 text-sm text-zinc-600 bg-zinc-50">
      {children}
    </div>
  );
}
...
function Logo() {
  return (
    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 grid place-items-center text-white font-bold">M</div>
  );
}

// ---------------------------
// Offline Montessori KB (condensed)
// ---------------------------
const KB = /** @type {Record<string, any>} */ ({
  "Pink Tower": {
...
const aliasIndex = Object.fromEntries(
  Object.entries(KB).flatMap(([name, v]) => v.aliases.map((a) => [a.toLowerCase(), name]))
);
...
// ---------------------------
// File input cluster (Files / Camera / Drag‑drop)
// ---------------------------
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 gap-2">
        <label className="block text-sm">
          <span className="block mb-1">Upload from files</span>
          <Input type="file" accept="image/*" onChange={onFile} />
        </label>
        <label className="block text-sm">
          <span className="block mb-1">Use camera</span>
          <Input type="file" accept="image/*" capture="environment" onChange={onFile} />
        </label>
        <div
          className="border-2 border-dashed border-zinc-300 rounded-xl p-3 text-center text-sm text-zinc-600"
...
export default function App() {
  ...
  const [model, setModel] = useState("gpt-4o-mini"); // vision + chat capable
  const [img, setImg] = useState(/** @type {string|null} */ (null));
  const [guess, setGuess] = useState(/** @type {string|null} */ (null));
  const [loadingDetect, setLoadingDetect] = useState(false);
  const [error, setError] = useState("");
  const [selected, setSelected] = useState(/** @type {string|null} */ (null));
  const [videos, setVideos] = useState(/** @type {string[]} */ ([]));

  // Chat state
  const [chatInput, setChatInput] = useState("");
  const [chatLoading, setChatLoading] = useState(false);
  const [messages, setMessages] = useState(/** @type {{role:'user'|'assistant', content:string}[]} */ ([
    { role: "assistant", content: "Hi! I’m your Montessori coach. Ask me about presentations, extensions, and scaffolding." },
  ]));
  ...
  async function onFile(e) {
    ...
    try {
      ...
      setSelected(null);
      if (apiKey) {
        await runVisionDetect(url);
      } else {
        const name = f.name?.toLowerCase() || "";
        const hit = Object.keys(aliasIndex).find((a) => name.includes(a));
        if (hit) {
          const canonical = aliasIndex[hit];
          setGuess(canonical);
          setSelected(canonical);
          setVideos(makeVideoQueries(canonical));
        } else {
          setGuess(null);
        }
      }
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a Montessori material identifier. Return ONLY the best‑guess canonical material NAME from this list if applicable: " +
                ALL_MATERIALS.join(", ") +
            ...
            },
            {
              role: "user",
              content: [
                { type: "text", text: "Identify the Montessori material in this image by its canonical name." },
  ...
  async function sendChat() {
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a certified Montessori guide (3-6 focus). Give concise, classroom-ready advice, cite control of error and indirect aims when relevant. Be supportive, specific, and developmentally appropriate.",
            },
            ...(selected
              ? [{ role: "system", content: `The current material is: ${selected}. Use its standard presentation unless asked otherwise.` }]
              : []),
            ...messages,
            userMsg,
          ],
      ...
      setMessages((m) => [
        ...
        {
          role: "assistant",
          content: `Hmm, I couldn't reach the AI right now. ${String(e instanceof Error ? e.message : e)}`,
        },
      ]);
    } finally {
  ...
  function offlineCoach(name, q) {
    const kb = KB[name];
    if (!kb) return "I couldn't find that in my offline set. Try another material.";
    const lower = q.toLowerCase();
    if (lower.includes("present") || lower.includes("how to") || lower.includes("steps")) {
      return `Presentation for ${name}:\n• ${kb.steps.join("\n• ")}`;
    }
    if (lower.includes("control of error") || lower.includes("error")) {
      return `Control of error for ${name}: ${kb.control}`;
    }
    if (lower.includes("aim") || lower.includes("goal")) {
      return `Direct aims: ${kb.aims.join(", ")}`;
    }
    if (lower.includes("age") || lower.includes("when")) {
      return `${name} typical age range: ${kb.age}`;
    }
    if (lower.includes("extension") || lower.includes("challenge")) {
      return `Extensions for ${name}: ${kb.extensions.join(", ")}`;
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 to-white text-zinc-900">
      <header className="sticky top-0 z-10 backdrop-blur bg-white/80 border-b border-zinc-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Logo />
            <div>
              <h1 className="text-xl font-semibold tracking-tight">Montessori Lesson Helper</h1>
              <p className="text-sm text-zinc-600 -mt-0.5">Snap a material → get presentation steps, videos, and coaching</p>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-4">
            <SectionTitle right={<Badge>{apiKey ? "AI Vision on" : "Offline mode"}</Badge>}>
              Capture or Upload
            </SectionTitle>
            <div className="grid md:grid-cols-[1fr,280px] gap-4 items-start">
              ...
              >
                {img ? (
                  <div className="relative aspect-[16/10] bg-zinc-100 rounded-xl overflow-hidden border border-zinc-200">
                    {/* eslint-disable-next-line jsx-a11y/alt-text */}
                    <img src={img} alt="Uploaded" className="w-full h-full object-contain" />
                  </div>
                ) : (
                  <div className="aspect-[16/10] rounded-xl border-2 border-dashed border-zinc-300 grid place-items-center text-zinc-500">
                  ...
                  {loadingDetect ? (
                    ...
                    <div>
                      <div className="font-medium">
                        Detected: <span className="text-indigo-700">{guess}</span>
                      </div>
                      <div className="mt-2">
                        <Button onClick={() => setSelected(guess!)} className="mr-2">
                          Use this
                        </Button>
                    ...
                    <span>
                      {apiKey
                        ? "Upload to auto‑identify."
                        : "Tip: Add an API key in Settings for auto‑ID, or pick from the list below."}
                    </span>
                  )}
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 max-h-48 overflow-auto pr-1">
                    {ALL_MATERIALS.map((m) => (
                      <button
                        ...
                        onClick={() => {
                          setSelected(m);
                          setVideos(makeVideoQueries(m));
                        }}
                        className={`text-left text-sm px-3 py-2 rounded-lg border ${
                          selected === m ? "border-indigo-500 bg-indigo-50" : "border-zinc-300 hover:bg-zinc-50"
                        }`}
                      >
                        {m}
                      </button>
          ...
          <Card className="p-4">
            ...
            {selected ? (
              <div className="space-y-4">
                ...
                <div>
                  <h4 className="font-medium mb-2">How to present</h4>
                  <ol className="list-decimal ml-5 leading-7 text-zinc-800">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    <h4 className="font-medium mb-1">Direct aims</h4>
                    <ul className="list-disc ml-5 text-zinc-800">
                      {material?.aims?.map((a, i) => (
                        <li key={i}>{a}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <h4 className="font-medium mb-1">Control of error</h4>
                    <p className="text-zinc-800">{material?.control}</p>
                  </div>
                  <div>
                    <h4 className="font-medium mb-1">Extensions</h4>
                    <ul className="list-disc ml-5 text-zinc-800">
                      {material?.extensions?.map((x, i) => (
                        <li key={i}>{x}</li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {videos.map((v, i) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block p-3 rounded-xl border border-zinc-300 hover:bg-zinc-50"
                  >
                    <div className="text-sm font-medium">YouTube search</div>
                    <div className="text-xs text-zinc-600 mt-1 break-all">{v}</div>
                  </a>
                ))}
              </div>
            ) : (
        ...
        <div className="space-y-6">
          <Card className="p-4 h-full">
            <SectionTitle right={<Badge>{apiKey ? "AI Chat on" : "Offline chat"}</Badge>}>
              Montessori Coach
            </SectionTitle>
            <div className="h-[420px] border border-zinc-200 rounded-xl overflow-hidden flex flex-col">
              <div className="flex-1 overflow-auto p-3 space-y-3 bg-zinc-50">
                {messages.map((m, i) => (
                  <div
                    key={i}
                    className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm leading-6 ${
                      m.role === "assistant" ? "bg-white border border-zinc-200" : "bg-indigo-600 text-white ml-auto"
                    }`}
                  >
                    {m.content}
                  </div>
              ...
              <div className="p-2 border-t border-zinc-200 bg-white">
                ...
                >
                  ...
                  />
                  <PrimaryButton type="submit" disabled={chatLoading}>
                    {chatLoading ? "Sending…" : "Send"}
                  </PrimaryButton>
                </form>
                {!apiKey && (
                  <p className="text-xs text-zinc-500 mt-2">
                    Tip: Add an OpenAI API key in Settings for deeper, personalized coaching.
                  </p>
                )}
              </div>
            </div>
          ...
          <Card className="p-4">
            <SectionTitle>Test Panel</SectionTitle>
            <Tests aliasIndex={aliasIndex} offlineCoach={offlineCoach} makeVideoQueries={makeVideoQueries} />
          </Card>
        </div>
      </main>
      ...
      <footer className="max-w-6xl mx-auto px-4 pb-10 text-xs text-zinc-500">
        <p>
          Built for educators. Offline knowledge is adapted from widely used Montessori training references; always align
          with your program’s scope & sequence.
        </p>
      </footer>
    </div>
  );
...
function ApiKeyPopover({ apiKey, setApiKey, model, setModel }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[360px] p-4 bg-white border border-zinc-200 rounded-2xl shadow-xl z-50">
          ...
          <label className="block text-sm mb-3">
            <span className="block text-zinc-700 mb-1">OpenAI API key (optional)</span>
            <Input value={localKey} onChange={(e) => setLocalKey(e.target.value)} placeholder="sk-..." />
            <span className="block text-xs text-zinc-500 mt-1">Stored in memory only during this session.</span>
          </label>
          <label className="block text-sm mb-3">
            <span className="block text-zinc-700 mb-1">Model</span>
            <select
              value={localModel}
              onChange={(e) => setLocalModel(e.target.value)}
              className="w-full h-11 rounded-xl border border-zinc-300 bg-white px-3"
            >
...
// ---------------------------
function Tests({ aliasIndex, offlineCoach, makeVideoQueries }) {
  ...
  function run() {
    ...
    // Test 1: aliasIndex maps known alias
    try {
      out.push({ name: "aliasIndex maps 'pink tower'", pass: aliasIndex["pink tower"] === "Pink Tower" });
    } catch (e) {
      out.push({ name: "aliasIndex error", pass: false, details: String(e) });
    }

    // Test 2: offlineCoach returns present steps
    try {
      const txt = offlineCoach("Pink Tower", "present");
      out.push({ name: "offlineCoach present includes bullet list", pass: txt.includes("Presentation for Pink Tower") && txt.includes("•") });
    } catch (e) {
      out.push({ name: "offlineCoach error", pass: false, details: String(e) });
    }

    // Test 3: makeVideoQueries returns two YouTube URLs
    try {
      const qs = makeVideoQueries("Pink Tower");
      out.push({ name: "makeVideoQueries length 2", pass: Array.isArray(qs) && qs.length === 2 });
      out.push({ name: "YouTube URL format", pass: qs.every((u) => typeof u === "string" && u.startsWith("https://www.youtube.com/results?search_query=")) });
    } catch (e) {
      out.push({ name: "makeVideoQueries error", pass: false, details: String(e) });
    }
  ...
  return (
    <div>
      ...
      <ul className="mt-3 space-y-1 text-sm">
        {results.map((r, i) => (
          <li key={i} className={r.pass ? "text-emerald-600" : "text-rose-600"}>
            {r.pass ? "✓" : "✗"} {r.name}
            {r.details ? <span className="text-xs text-zinc-500"> — {r.details}</span> : null}
          </li>
        ))}
      </ul>
    </div>
